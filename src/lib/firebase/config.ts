...
// 後方互換性のため、プロパティアクセスで遅延初期化（非同期版）
export const app = new Proxy({} as FirebaseApp, {
  get(_target, prop) {
    if (!_app) {
      throw new Error('Firebase not initialized. Use getFirebaseApp() instead.');
    }
    return _app[prop as keyof FirebaseApp];
  }
});
...
export const db = new Proxy({} as Firestore, {
  get(_target, prop) {
    if (!_db) {
      throw new Error('Firestore not initialized. Use getFirebaseDb() instead.');
    }
...
export const storage = new Proxy({} as FirebaseStorage, {
  get(_target, prop) {
    if (!_storage) {
      throw new Error('Storage not initialized. Use getFirebaseStorage() instead.');
    }
...
export const auth = new Proxy({} as Auth, {
  get(_target, prop) {
    if (!_auth) {
      throw new Error('Auth not initialized. Use getFirebaseAuth() instead.');
    }
